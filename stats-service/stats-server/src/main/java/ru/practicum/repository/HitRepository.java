...
public interface HitRepository extends JpaRepository<Hit, Long> {
    @Query("SELECT NEW ru.practicum.dto.StatsDto(h.app, h.uri, COUNT(h)) FROM Hit h " +
            "WHERE h.timestamp BETWEEN :start AND :end GROUP BY h.app, h.uri ORDER BY COUNT(h) DESC")
    List<StatsDto> countAllBetween(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query("SELECT NEW ru.practicum.dto.StatsDto(h.app, h.uri, COUNT(h)) FROM Hit h " +
            "WHERE h.timestamp BETWEEN :start AND :end AND h.uri IN (:uris) GROUP BY h.app, h.uri ORDER BY COUNT(h) DESC")
    List<StatsDto> countAllByUrisBetween(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end,
